...
const schema = z.object({
    productType: z
        .string()
        .min(2, {
            message: 'O grupo do produto deve ter pelo menos 2 caracteres.',
        })
        .max(60, { message: 'Seleciona um tipo valido.' }),
    productCode: z
        .string()
        .min(3, {
            message: 'O código do produto deve ter entre 3 e 60 caracteres.',
        })
        .max(60),
    productGroup: z
        .string()
    ...
    productNumberCode: z
        ...
        .min(3, {
            message:
                'O número do código do produto deve ter entre 3 e 60 caracteres.',
        })
        .max(60),
    productDescription: z
        ...
        .min(3, {
            message:
                'A descrição do produto deve ter entre 3 e 200 caracteres.',
        })
        .max(200),
    taxIva: z
        .string()
        .min(1, { message: 'Taxa do IVA obrigatória.' })
        .transform((val) => {
    ...
    newProductType: z
        .string()
        .min(2, {
            message: 'O tipo do produto deve ter pelo menos 2 caracteres.',
        })
        .max(60, { message: 'Seleciona um tipo valido.' }),
...
const ProdutoForm = () => {
    ...
    const [mensagemDeSucessoNoFront, setMensagemDeSucessoNoFront] = useState(
        []
    );
    var sucesso = [];
    var erros = [];
    ...
    const [propsNewType, setPropsNewType] = useState(false);
    ...
    const buscarProdutosGrupos = async () => {
        ...
        try {
            ...
            const grupos = [
                ...new Set(
                    result.data.map((produto) => produto.designacaoProduto)
                ),
            ];
            ...
            console.log(
                'Erro ao efetuar a chamada da API para grupos de produtos',
                error
            );
            erros.push('Erro Interno do Servidor');
        } finally {
    ...
    const buscarTiposProduto = async () => {
        ...
        try {
            ...
            const tipos = [
                ...new Set(
                    result.data.map((produto) => produto.designacaoTipoProduto)
                ),
            ];
            ...
            console.log(
                'Erro ao efetuar a chamada da API para tipos de produtos',
                error
            );
            erros.push('Erro Interno do Servidor');
        } finally {
    ...
    const onConfirmar = async (data) => {
        ...
        const dataSubmit = {
            ...produtoRemover,
            status: false,
            id: produtoRemover.id,
        };
        const response = await api
            .put('produto/del', dataSubmit)
            .then((result) => {
            ...
            .catch((error) => {
                console.log('Erro ao remover o produto ', d.id);
    ...
    const filteredProducts = produto.filter(
        (prod) =>
            prod.productDescription
                .toLowerCase()
                .includes(searchTerm.toLowerCase()) // Filtra pela descrição do produto
    );
                ...
                <table border="1">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>Código Número do Produto</th>
                        ...
                        {currentProducts.length > 0 ? (
                            currentProducts.map((produto, index) => (
                                <tr key={index}>
                                    ...
                                    <td>{produto.productDescription}</td>
                                    <td>{produto.productGroup}</td>
                                    <td>{produto.productNumberCode}</td>
                                    <td>{produto.productType}</td>
                                    <td>{produto.taxIva}</td>
                                    <td>
                                        <button
                ...
                >
                    ...
                    <div className="input-group">
                        ...
                        <div className="select-container">
                            <select
                                ...
                                className={
                                    errors.productType ? 'input-error' : ''
                                }
                            >
                                <option value="">
                            ...
                            <div className="action-buttons">
                                <ProdutoTypeForm
                                    buscarTiposProduto={buscarTiposProduto}
                                />
                            </div>
                        </div>
                    ...
                    <div className="input-group">
                        ...
                        <div className="select-container">
                            ...
                            <div className="action-buttons">
                                <ProdutoGroupForm
                                    buscarProdutosGrupos={buscarProdutosGrupos}
                                />
                            </div>
                        </div>
                    ...
                    {/* Número do Código */}
                    <div className="input-group">
                        <label htmlFor="productNumberCode">
                            Número do Código
                        </label>
                        <input
                            type="text"
                            ...
                            className={
                                errors.productNumberCode ? 'input-error' : ''
                            }
                        />
                        {errors.productNumberCode && (
                            <span className="error-message">
                                {errors.productNumberCode.message}
                            </span>
                        )}
                    </div>
